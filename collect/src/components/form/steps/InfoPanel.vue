  ...
  <q-card flat v-if="withInfo">
    <q-card-section>
      <div class="row items-start">
        <q-icon name="info" color="primary" size="sm" class="q-mr-md q-mt-xs" />
        <div>
          <div class="question-label text-bold q-mb-xs">
            {{ t('contact.header', { company_name: collector.info.company_name }) }}
          </div>
          <div v-if="hasContact" class="question-hint">
            {{
...
const hasContact = computed(() => collector.info.contact_name || collector.info.contact_email)

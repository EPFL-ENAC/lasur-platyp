<template>
  <div>
    <SectionItem :label="t('form.change')" label-class="text-h4" class="q-mb-md" />
    <q-card class="bg-primary-muted q-mb-xl">
      <q-card-section class="q-pa-sm">
        <div class="text-h5 text-secondary text-center">
          {{ t(`reco.${scndRecoDt}`) }}
        </div>
      </q-card-section>
    </q-card>

    <div>
      <RatingItem
        v-if="isRecoChange2"
        :label="t('form.change_motivation')"
        :hint="t('form.change_motivation_hint')"
        v-model="survey.record.data.change2.motivation"
        :max="5"
        label-class="text-h4 text-bold q-mb-md"
        class="q-mb-lg"
        @update:model-value="onSave"
      />
      <ChoiceItem
        :label="t('form.change_levers')"
        :options="[
          { value: 'finance', label: t('form.change_levers_option.financial_support') },
          { value: 'flexibility', label: t('form.change_levers_option.work_flexibility') },
          { value: 'collective', label: t('form.change_levers_option.collective_changes') },
          { value: 'environment', label: t('form.change_levers_option.work_environment') },
          { value: 'other', label: t('form.change_levers_option.other') },
        ]"
        v-model="survey.record.data.change2.levers"
        multiple
        label-class="text-h4 text-bold q-mb-md"
        option-label-class="text-h5"
        @update:model-value="onSave"
      />
      <q-input
        v-if="survey.record.data.change2.levers?.includes('other')"
        v-model="survey.record.data.change2.other_levers"
        :label="t('form.change_other_levers_specify')"
        type="textarea"
        class="q-mb-lg text-h6"
        color="field"
        bg-color="field"
        outlined
        rounded
        debounce="500"
        @update:model-value="onSave"
      />
    </div>
  </div>
</template>

<script setup lang="ts">
import SectionItem from 'src/components/form/SectionItem.vue'
import RatingItem from 'src/components/form/RatingItem.vue'
import ChoiceItem from 'src/components/form/ChoiceItem.vue'

const { t } = useI18n()
const survey = useSurvey()

const emit = defineEmits(['update:modelValue'])

const isRecoChange2 = computed(() => !survey.isRecommendation2InUse())

const scndRecoDt = computed(() =>
  survey.recommendation.reco && survey.recommendation.reco.reco_inter.length > 1
    ? survey.recommendation.reco.reco_inter[1]
    : '',
)

function onSave() {
  emit('update:modelValue')
}
</script>

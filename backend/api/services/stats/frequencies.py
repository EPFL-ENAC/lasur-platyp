...
class FrequenciesService(BaseStatsService):
    ...
    PRO_JOURNEY_DAYS_PATTERN = re.compile(r"^data\.freq_mod_pro_journeys\..*\.days$")
    JOURNEY_MODES_PATTERN = re.compile(r"^data\.freq_mod_journeys\.(\d+)\.modes\..*$")
    ...
    def compute_modes_frequencies(self) -> list[Frequencies]:
        ...
        if not df_v2.empty:
            results_v2 = []
            for mode in MODES:
                results_v2.append(self._compute_mode_frequencies_v2(df_v2, mode))
    ...
    def compute_modes_pro_frequencies(self) -> list[Frequencies]:
        ...
        if not df_v2.empty:
            ...
            for mode in MODES_PRO:
                results_v2.extend(self._compute_mode_pro_frequencies_v2(df_v2, mode))
            results = self._merge_frequencies(results, results_v2)
        # finalize totals and sort data
        for frequencies in results:
            frequencies.total = len(self.df)
    ...
    def _compute_mode_pro_frequencies_v2(
        ...
        for i in range(len(col_days)):
            ...
            for idx, row in df_i.iterrows():
                ...
                freq = next((f for f in frequencies if f.value == str(days)), None)
                if freq is None:
                    frequencies.append(
                        Frequency(value=str(days), count=int(count), sum=int(days))
                    )
                else:
    ...
    def _compute_mode_frequencies_v2(self, df: pd.DataFrame, mode: str) -> Frequencies:
        ...
        col_days = [col for col in df.columns if self.JOURNEY_DAYS_PATTERN.match(col)]
        # print(
        #     f"Computing mod frequencies for version 2.x using columns: {col_days.tolist()}")
        frequencies = []
        for i in range(len(col_days)):
            ...
            col_modes_i = df.columns[
                df.columns.str.startswith(f"data.freq_mod_journeys.{str(i)}.modes.")
            ]
            ...
            df_i["inter"] = df_i.apply(lambda row: is_intermodal(row, i), axis=1)
            # extract mod days

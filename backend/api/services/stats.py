...
class StatsService:
    ...
    def compute_stats(self, df: pd.DataFrame) -> Stats:
        ...
        pro_mode_frequencies = self.compute_modes_pro_frequencies(df)
        pro_mode_emissions = self.compute_modes_pro_emissions(df)
        pro_recommendations = self.compute_recommendation_pro_frequencies(df)
        pro_mode_links = self.compute_mode_reco_pro_links(df)
    ...
    def compute_mode_reco_pro_links(self, df: pd.DataFrame) -> Links:
        """Compute all mode recommendation links from a DataFrame of records."""
        # v1: legacy data version
        df_v1 = self._get_records_v1(df)

...
@pytest.mark.asyncio
async def test_compute_equipments_frequencies():
    ...
    result = await service.compute_equipments_frequencies(df)
    ...
    expected = Frequencies(
        ...
        data=[
            ...
            Frequency(value='mob_subs', count=7, sum=None),
            Frequency(value='train_subs', count=6, sum=None),
...
@pytest.mark.asyncio
async def test_compute_constraints_frequencies():
    # Load the test CSV into a DataFrame
    df = pd.read_csv('tests/data/records.csv')
    service = RecordService(session=None)  # session not needed for this test
    df = service.filter_completed_records(df)
    result = await service.compute_constraints_frequencies(df)
    ...
    expected = Frequencies(
        ...
        total=28,
        data=[
            Frequency(value='night', count=8, sum=None),
            Frequency(value='disabled', count=6, sum=None),
            Frequency(value='heavy', count=4, sum=None),
...
@pytest.mark.asyncio
async def test_compute_travel_time_frequencies():
    # Load the test CSV into a DataFrame
    df = pd.read_csv('tests/data/records.csv')
    service = RecordService(session=None)  # session not needed for this test
    df = service.filter_completed_records(df)
    result = await service.compute_travel_time_frequencies(df)
    ...
    expected = Frequencies(
        ...
        data=[
            ...
            Frequency(value='35', count=3, sum=None),
...
@pytest.mark.asyncio
async def test_compute_recommendation_frequencies():
    # Load the test CSV into a DataFrame
    df = pd.read_csv('tests/data/records.csv')
    service = RecordService(session=None)  # session not needed for this test
    df = service.filter_completed_records(df)
    result = await service.compute_recommendation_frequencies(df)
    ...
    expected = Frequencies(
        ...
        data=[
            ...
            Frequency(value='inter', count=4, sum=None),
            Frequency(value='train', count=3, sum=None),
            Frequency(value='vae', count=3, sum=None),
            Frequency(value='tpu', count=3, sum=None),
